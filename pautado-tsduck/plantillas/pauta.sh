...
# ===== Config =====
RC_HOST="127.0.0.1"
TELNET_PORT="{{ TELNET_PORT }}"  # Puerto del Remote Control
RC_PASS="pautado"
SPOT_DEF="SPOT1"
DUR_DEF=60
...
need nc
ss -ltn | grep -q ":${TELNET_PORT}\b" || { echo "RC ${TELNET_PORT} no abierto"; exit 1; }
...
rc() {
  {
    printf "%s\r\n" "$RC_PASS"
    printf "%s\r\n" "$*"
  } | nc $NC_OPTS "$RC_HOST" "$TELNET_PORT" >/dev/null 2>&1 || true
}
...
rc_out() {
  ...
  out="$({
    printf "%s\r\n" "$RC_PASS"
    printf "%s\r\n" "$cmd"
  } | nc $NC_OPTS "$RC_HOST" "$TELNET_PORT" || true)"
  say "RC> $cmd"
